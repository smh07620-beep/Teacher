...
(function(){
  ...
  window.renderAdminMaterials = async function(force=false){
    ...
    try {
      ...
      await window.hydrateMaterialIndexStatus(materials);
    } catch (err) {
      box.innerHTML = `<p class="text-xs text-rose-500">❌ ${escapeHtml(err.message)}</p>`;
    }
  };

  window.hydrateMaterialIndexStatus = async function(materials){
    for(const m of materials.filter(x=>!x.isBuiltin)){
      ...
      try{
        ...
        el.textContent=`全文搜尋：${label} · ${Number(d.page_count||0)} 頁${d.last_indexed_at?` · ${d.last_indexed_at}`:''}`;
      }catch(_){
        el.textContent='全文搜尋：狀態無法讀取';
      }
    }
  };
