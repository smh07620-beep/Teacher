 ...
 * inline-handler and sibling-script contracts during the incremental split.
 */
(function(){
  'use strict';
  ...
  function openAdminUserEditor(username){
    ...
    document.getElementById('admin-user-editor-status').textContent='';
    adminSyncProfileTagChecks();
  ...
  async function saveAdminUserEditor(){
    ...
    try{
      ...
      if(status) status.textContent='✅ 人員資料已儲存；系統角色與權限未變更。';
