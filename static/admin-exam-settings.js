...
(() => {
  ...
  async function openSettings(catId) {
    ...
    try {
      ...
      document.getElementById('exam-reviewer-name').value = category.reviewerName || readLocalMemory(EVALUATOR_NAME_MEMORY_KEY) || '';
  ...
  async function review() { const catId = document.getElementById('exam-settings-id').value, reviewerName = document.getElementById('exam-reviewer-name').value.trim(); if (!catId) return; if (!reviewerName) { status().textContent = '❌ 請填寫審核者姓名'; return; } await preview(); const key = await getAdminKey(); if (!key) return; try { status().textContent = '⏳ 正在檢查題目完整性並送出審核…'; const response = await fetch(`/api/quiz-categories/${catId}/review`, {method:'POST', headers:{'Content-Type':'application/json','X-Admin-Key':key}, body:JSON.stringify({reviewerName})}), data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.issues?.length ? `${data.error}：${data.issues.join('、')}` : (data.error || '審核失敗')); editingMeta = {...(editingMeta || {}), reviewStatus:'approved', reviewerName, reviewedAt:data.reviewedAt, active:false}; status().textContent = `✅ 審核完成：${reviewerName}；現在可發布考卷。`; updateWorkflow(editingMeta); adminQuizCategoriesCache.clear(); } catch (error) { status().textContent = '❌ ' + error.message; } }
  async function publish() { const catId = document.getElementById('exam-settings-id').value; if (!catId) return; const key = await getAdminKey(); if (!key) return; try { status().textContent = '⏳ 正在建立不可漂移發布快照並發布…'; const response = await fetch(`/api/quiz-categories/${catId}/publish`, {method:'POST', headers:{'X-Admin-Key':key}}), data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || '發布失敗'); editingMeta = {...(editingMeta || {}), active:true, publishedAt:data.publishedAt, reviewStatus:'approved', publicationId:data.publicationId || '', publicationHash:data.publicationHash || ''}; status().textContent = `🚀 考卷已發布；已保存 ${Number(data.snapshotQuestionCount || 0)} 題發布快照${data.publicationHash ? '（' + data.publicationHash.slice(0,10) + '…）' : ''}。`; updateWorkflow(editingMeta); adminQuizCategoriesCache.clear(); Object.keys(dynamicCategoriesCache).forEach(key => delete dynamicCategoriesCache[key]); } catch (error) { status().textContent = '❌ ' + error.message; } }
