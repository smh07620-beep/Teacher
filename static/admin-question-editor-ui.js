    ...
    return `<div id="qrow-${q.id}" class="border ${q.active===false?'border-amber-200 bg-amber-50/50':'border-slate-200 bg-white'} rounded-xl p-3 overflow-visible">
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="w-full min-w-0 flex-1 text-xs flex items-start gap-2.5">
          <input type="checkbox" class="qselect-${catId} mt-1 rounded shrink-0" data-qid="${q.id}" data-csp-change="adminUpdateQuestionSelection('${catId}')">
        ...
        <div class="w-full sm:w-auto flex items-center justify-end gap-2 shrink-0">
          <button data-csp-click="adminToggleInlineQuestionEditor('${q.id}','${catId}',true)" class="text-[11px] bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-2 rounded-lg font-bold">✏️ 編輯</button>
          <details class="relative"><summary class="list-none cursor-pointer text-lg leading-none bg-white border border-slate-300 text-slate-600 px-3 py-1.5 rounded-lg" aria-label="更多題目操作">⋯</summary><div class="absolute right-0 z-40 mt-1 w-36 rounded-xl border border-slate-200 bg-white p-2 shadow-xl space-y-1"><button data-csp-click="adminToggleQuizQuestion('${q.id}','${catId}',${q.active===false?'true':'false'})" class="w-full text-left text-[11px] ${q.active===false?'text-emerald-700 hover:bg-emerald-50':'text-amber-700 hover:bg-amber-50'} px-2 py-2 rounded-lg">${q.active===false?'▶ 啟用':'⏸ 停用'}</button><button data-csp-click="adminDeleteQuizQuestion('${q.id}','${catId}')" class="w-full text-left text-[11px] text-rose-700 hover:bg-rose-50 px-2 py-2 rounded-lg">🗑️ 刪除</button></div></details>
        </div>
