...
(function(){
  ...
  function externalProvider(url){try{const u=new URL(url);if(u.protocol!=='https:')return {ok:false,label:'僅接受 HTTPS'};const host=u.hostname.toLowerCase();if(host==='youtube.com'||host==='www.youtube.com'||host==='m.youtube.com'||host==='youtu.be'||host==='youtube-nocookie.com'||host==='www.youtube-nocookie.com')return {ok:true,label:u.pathname.includes('/shorts/')?'YouTube Shorts':'YouTube'};if(host==='vimeo.com'||host==='www.vimeo.com'||host==='player.vimeo.com')return {ok:true,label:'Vimeo'};if(/\.(mp4|webm)$/i.test(u.pathname))return {ok:true,label:'院內 CDN（仍需伺服器 allowlist 驗證）'};return {ok:false,label:'僅接受 YouTube、Vimeo 或 allowlisted 院內 CDN MP4/WebM'};}catch(_){return {ok:false,label:'網址格式不正確'};}}
