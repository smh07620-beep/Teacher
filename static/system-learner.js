...
function buildSlideCardHTML(s) {
    ...
    const jumpBtn = s.category
        ...
        ? `<button data-csp-click="goToExamModule('${s.category}', '${groupKey}')" class="flex-1 text-xs font-semibold bg-teal-600 hover:bg-teal-500 text-white py-2 rounded-lg transition-colors flex items-center justify-center gap-1">📝 前往對應考題作答</button>`

        : `<span class="flex-1 text-xs text-slate-400 flex items-center justify-center">未連結考題模組</span>`;

    const openBtn = `<button data-csp-click="openMaterial('${s.id}')" class="flex-1 text-xs font-bold bg-[#006b64] hover:bg-[#005a54] text-white py-2.5 rounded-lg transition-colors flex items-center justify-center gap-1">📖 開始閱讀</button>`;

    const deleteBtn = s.isBuiltin

        ? ''

        : `<button data-csp-click="deleteUploadedSlide('${s.id}')" class="text-xs font-semibold text-rose-600 hover:text-rose-800 px-2 py-1 rounded-lg transition-colors">🗑️ 刪除</button>`;
    ...
    const completeBtn = s.isBuiltin ? '' : `<button data-csp-click="markMaterialComplete('${s.id}')" class="w-full mt-2 text-xs font-semibold ${done?'bg-emerald-100 text-emerald-800 border-emerald-200':'bg-white text-emerald-700 border-emerald-300'} border py-2 rounded-lg">${done?'✅ 已完成教材':'☑️ 標記教材完成'}</button>`;
...
function buildAtlasCardHTML(m){const src=m.viewUrl||`/view/${encodeURIComponent(m.id)}`;const meta=m.atlasMeta||{};const chips=[meta.category,meta.magnification,meta.normality].filter(Boolean).map(x=>`<span class="atlas-chip">${escapeHtml(x)}</span>`).join('');const tagchips=(meta.tags||'').split(',').map(x=>x.trim()).filter(Boolean).slice(0,5).map(x=>`<span class="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">#${escapeHtml(x)}</span>`).join('');return `<article class="atlas-card micro-card"><button data-csp-click="openAtlas('${m.id}')" class="atlas-preview w-full text-left"><img src="${src}" loading="lazy" decoding="async" alt="${escapeHtml(m.title||'Atlas 圖譜')}"><span class="absolute right-2 bottom-2 px-2 py-1 rounded-lg bg-black/65 text-white text-[11px] font-bold">🔍 點擊放大</span></button><div class="p-4"><div class="flex items-center gap-2 flex-wrap"><span class="atlas-chip">Atlas</span>${chips}${tagchips}<span class="text-[11px] text-slate-400">高解析原圖</span></div><h4 class="font-black text-slate-900 mt-2">${escapeHtml(m.title||m.filename)}</h4><p class="text-xs text-slate-500 mt-1 leading-5">${escapeHtml(meta.interpretation||m.desc||'適合鏡檢照片、結晶、細胞型態與微生物圖像判讀。')}</p><button data-csp-click="openAtlas('${m.id}')" class="micro-btn mt-3 w-full bg-[#006b64] text-white rounded-lg py-2 text-xs font-bold">🔬 檢視細節</button></div></article>`;}
...
function renderSlideThumbs() {
    ...
    if(slideViewerState.mode==='pdf'){
        ...
        container.innerHTML=Array.from({length:total},(_,i)=>`<button class="slide-page-btn ${i===slideViewerState.index?'active':''}" data-csp-click="goToSlidePage(${i})" aria-label="跳到第 ${i+1} 頁">${i+1}</button>`).join('');
    ...
    container.innerHTML = slideViewerState.images.map((src, i) => `
        ...
        <button class="slide-thumb-card ${i===slideViewerState.index?'active':''}" data-csp-click="goToSlidePage(${i})" aria-label="跳到第 ${i + 1} 頁">
...
async function openCourseFeedback(courseId,courseTitle){
    ...
    document.getElementById('course-feedback-course-id').value=courseId||'';document.getElementById('course-feedback-title').textContent=courseTitle||'課程';document.getElementById('course-feedback-rating').value='';document.getElementById('course-feedback-comment').value='';if(status)status.textContent='正在讀取你先前的回饋…';
    if(typeof dialog.showModal==='function'&&!dialog.open)dialog.showModal();
    try{const res=await fetch(`/api/course-feedback/${encodeURIComponent(courseId)}`,{credentials:'same-origin',cache:'no-store'});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'無法讀取課程回饋');const feedback=data.feedback||null;if(feedback){document.getElementById('course-feedback-rating').value=String(feedback.rating||'');document.getElementById('course-feedback-comment').value=feedback.comment||'';if(status)status.textContent='已載入你上次的回饋，可直接修改後重新儲存。';}else if(status)status.textContent='尚未填寫回饋。';}catch(error){if(status)status.textContent=`❌ ${error.message}`;}
}

async function saveCourseFeedback(event){
    event?.preventDefault?.();const courseId=document.getElementById('course-feedback-course-id')?.value||'',rating=Number(document.getElementById('course-feedback-rating')?.value||0),comment=document.getElementById('course-feedback-comment')?.value||'',status=document.getElementById('course-feedback-status');if(!courseId)return;if(![1,2,3,4,5].includes(rating)){if(status)status.textContent='❌ 請先選擇 1 到 5 分。';return;}if(status)status.textContent='正在儲存回饋…';
    try{const res=await fetch(`/api/course-feedback/${encodeURIComponent(courseId)}`,{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({rating,comment})});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'儲存失敗');if(status)status.textContent='✅ 回饋已儲存，你之後仍可再次修改。';}catch(error){if(status)status.textContent=`❌ ${error.message}`;}
}

function bindCourseFeedbackUI(){
    const dialog=document.getElementById('course-feedback-dialog'),form=document.getElementById('course-feedback-form');if(!dialog||!form||form.dataset.feedbackBound==='1')return;form.dataset.feedbackBound='1';form.addEventListener('submit',saveCourseFeedback);document.getElementById('course-feedback-close')?.addEventListener('click',()=>dialog.close());document.getElementById('course-feedback-cancel')?.addEventListener('click',()=>dialog.close());
}
