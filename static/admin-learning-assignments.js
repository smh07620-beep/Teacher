...
(function(){
  ...
  const scopeNow=()=>({area:document.getElementById('wizard-area')?.value||window.currentTrainingArea||'internal',group:document.getElementById('wizard-group')?.value||window.currentGroupKey||'grpBio'});
  ...
  function mount(){
    ...
    panel=document.createElement('section');panel.id=PANEL_ID;panel.className='rounded-2xl border border-sky-100 bg-sky-50/40 p-4 mb-4';
    ...
    hub.before(panel);panel.querySelector('[data-la-refresh]').addEventListener('click',()=>load(true));return panel;
  }
  ...
  function assigneeLabel(item){if(item.assigneeType==='all')return'全體人員';if(item.assigneeType==='group')return`本組（${item.group||item.assigneeKey||''}）`;const user=item.assignee||{};return user.name?`${user.name}${user.empId?` · ${user.empId}`:''}`:(item.assigneeKey||'個人');}
  ...
  function render(){
    ...
    const type=body.querySelector('[data-la-type]'),userWrap=body.querySelector('[data-la-user-wrap]');type.addEventListener('change',()=>userWrap.classList.toggle('hidden',type.value!=='user'));
    body.querySelector('[data-la-form]').addEventListener('submit',submitAssignment);body.querySelectorAll('[data-la-deactivate]').forEach(button=>button.addEventListener('click',()=>deactivate(button.dataset.laDeactivate)));
  }

  async function loadCourses(){const {area,group}=scopeNow();const response=await fetch(`/api/courses/admin?area=${encodeURIComponent(area)}&group=${encodeURIComponent(group)}`,{credentials:'same-origin',cache:'no-store'});if(response.status===403){mount()?.classList.add('hidden');return false;}const data=await json(response);state.courses=Array.isArray(data)?data:[];mount()?.classList.remove('hidden');if(!state.courses.some(c=>c.id===state.courseId))state.courseId=state.courses[0]?.id||'';return true;}
  async function loadCourse(){if(!state.courseId){state.assignments=[];state.users=[];render();return;}const [assignments,candidates]=await Promise.all([fetch(`/api/learning-assignments?courseId=${encodeURIComponent(state.courseId)}`,{credentials:'same-origin',cache:'no-store'}).then(json),fetch(`/api/learning-assignments/candidates?courseId=${encodeURIComponent(state.courseId)}`,{credentials:'same-origin',cache:'no-store'}).then(json)]);state.assignments=Array.isArray(assignments.assignments)?assignments.assignments:[];state.users=Array.isArray(candidates.users)?candidates.users:[];state.canAssignAll=!!candidates.canAssignAll;render();}
  async function load(force=false){const panel=mount();if(!panel)return;const body=panel.querySelector('[data-la-body]');if(body&&(force||!state.courses.length))body.textContent='讀取課程指派中…';try{if(!(await loadCourses()))return;await loadCourse();}catch(error){if(body)body.innerHTML=`<div class="rounded-xl border border-rose-100 bg-rose-50 p-3 text-rose-700">❌ ${escapeHtml(error.message)}</div>`;}}
  ...
  async function submitAssignment(event){
    ...
    const dueAt=due?new Date(due).toISOString():'';
    state.busy=true;
    try{await json(await fetch('/api/learning-assignments',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({courseId:state.courseId,assigneeType:type,assigneeKey:type==='user'?user:type==='group'?scopeNow().group:'*',required,dueAt})}));await loadCourse();if(typeof window.renderAdminCourseMaterialHub==='function')window.renderAdminCourseMaterialHub(true).catch(()=>{});}catch(error){alert(error.message);}finally{state.busy=false;}
  }

  async function deactivate(id){if(!id||!confirm('取消這筆課程指派？既有學習紀錄不會刪除。'))return;try{await json(await fetch(`/api/learning-assignments/${encodeURIComponent(id)}`,{method:'DELETE',credentials:'same-origin'}));await loadCourse();}catch(error){alert(error.message);}}
