...
| Training Command Center / 我的待辦 (7.1 M1) | `teacher_app.command_center.service`, `teacher_app.command_center.routes`, `static/training-command-center-71.js` | `GET /api/training-command-center`; M1 scope is explicitly `pgy` | normal `/system` main-page 我的待辦 card; task actions navigate to the canonical PGY workflow instead of mutating in the dashboard | authenticated user; PGY assignment scope remains canonical; system admin/auditor receive no clinical-signing actions | `test_training_command_center_71.py`, executable RC matrix contract | Usable |
| PGY competency matrix / training progress (7.1 M2) | `teacher_app.command_center.competency`, `teacher_app.command_center.scope`, `teacher_app.command_center.routes`, `static/pgy-competency-matrix-71.js` | `GET /api/training-command-center/pgy-matrix`; read-only projection over PGY assignments and formal assessment tools | `/system` PGY 能力矩陣／訓練進度 surface | student=self; clinical teacher=assigned learners; group leader=own group; education admin=all learners; system admin/auditor excluded from learner analytics scope | `test_training_command_center_71_m2_m3.py` | Usable |
| Learning Analytics (7.1 M3) | `teacher_app.command_center.analytics`, `teacher_app.command_center.scope`, `teacher_app.command_center.routes`, `static/learning-analytics-71.js` | `GET /api/training-command-center/learning-analytics`; descriptive projection of existing learning progress, exam records, PGY assignments and PGY assessments | `/system` Learning Analytics surface with learner-level metrics and six-month activity timeline | same canonical learner visibility scope as M2; read-only; no synthetic mastery or new total ability score | `test_training_command_center_71_m2_m3.py` | Usable |
| Notification Center (7.1 M4) | `static/notification-center-71.js` read-only aggregation over canonical APIs | `/api/auth/me`, `/api/training-command-center`, `/api/dashboard/me`, `/api/announcements`; no new mutation API | `/system` 通知中心 surface aggregating PGY tasks, pending exams and platform announcements | authenticated session; each source keeps its existing server-side RBAC; notification layer grants no new signing, grading, progress or announcement-management authority | `test_training_command_center_71_m4.py`; release JS syntax gate | Usable |
