...
from teacher_app.config import storage_paths as canonical_storage_paths
...
# 路徑與基本設定
# ---------------------------------------------------------------------------
_STORAGE_PATHS = canonical_storage_paths(ensure=False)
BASE_DIR = _STORAGE_PATHS.base_dir
STATIC_DIR = _STORAGE_PATHS.static_dir
SLIDES_DIR = _STORAGE_PATHS.slides_dir
MATERIAL_STORAGE = _STORAGE_PATHS.material_storage
UPLOAD_DIR = _STORAGE_PATHS.upload_dir
QUESTION_IMAGES_DIR = _STORAGE_PATHS.question_images_dir
UPLOADED_SLIDES_DIR = _STORAGE_PATHS.uploaded_slides_dir
DOC_TEMPLATES_DIR = _STORAGE_PATHS.doc_templates_dir
PGY_ASSESSMENT_TEMPLATES_DIR = _STORAGE_PATHS.pgy_assessment_templates_dir
DATA_DIR = _STORAGE_PATHS.data_dir
META_FILE = DATA_DIR / "slides_meta.json"
TMP_DIR = _STORAGE_PATHS.tmp_dir
UPLOAD_PROGRESS_DIR = _STORAGE_PATHS.upload_progress_dir
PREVIEW_CACHE_DIR = _STORAGE_PATHS.preview_cache_dir
...
from teacher_app.auth import accounts as auth_accounts, service as auth_service, routes as auth_routes
from teacher_app.maintenance import migrations as schema_migrations
import sys
...
def init_user_accounts_db():
    """Explicit legacy compatibility hook; production schema uses migrations."""
    schema_migrations.ensure_user_accounts_base(sys.modules[__name__])
...
def api_users_admin():
    ...
    return jsonify(auth_accounts.list_accounts(sys.modules[__name__]))
...
def api_user_create():
    ...
    try:
        user = auth_accounts.create_account(sys.modules[__name__], data)
...
def api_user_update(username):
    ...
    data = request.get_json(silent=True) or {}
    try:
        user = auth_accounts.update_account(sys.modules[__name__], username, data)
        return jsonify({"ok": True, "user": user})
    except auth_accounts.AccountNotFound as exc:
