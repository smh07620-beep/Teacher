> 最新更新：V5.7.0 正式穩定版（大型教材背景佇列＋單一預覽 Fast Web View＋考卷發布快照）。請先閱讀 [更新與使用說明](README_V5_7_0.md)。

# 2026 生化教育訓練考核系統

> **目前版本：V5.7.0｜正式穩定版** － 大型教材背景佇列、單一 PDF 預覽加速、考卷發布快照與既有 V5.6.x 教學可信度規則。詳細變更請看 `README_V5_7_0.md`，部署前依 `TEST_CHECKLIST_V5_7_0.txt` 驗收。
...
- **1 生化組**：原有四份考卷已轉為資料庫動態考卷，可由管理後台編輯、停用、增題與 AI 出題
  - 一致性與法定傳染病通報
  - c503 一般作業流程與異常訊號故障排除
  - Cobas b 211 異常訊號故障排除與 QC 設定
  - COVER C1 / Sebia 人員考區
- **六組別**：投影片、課程、考題頁籤與題目皆可由管理者後台新增與維護
- 教育訓練 PPT/PPTX 線上瀏覽
- Office / PPT / PDF 上傳後由背景 Worker 建立單一 `preview.pdf`；新教材不再產生「每頁一張圖片」的碎片檔
- 各頁籤考試、計分與合格判定
- 附件 1 Word 匯出
- CSV 成績匯出（含組別欄位）
...
## V5.2 Cloudflare R2 教材儲存（舊版相容）
V5.2 可將教育訓練教材保存至 Cloudflare R2 Private Bucket。詳細建立步驟、Render 環境變數與舊教材搬移方式請參閱 `README_V5_2.md`。


## V5.3 Google Drive 教材儲存
最新版預設可將教育訓練教材保存到 Google Drive，Supabase PostgreSQL 繼續保存課程、題庫、成績、PGY 進度與問答題批改資料。Cloudflare R2 仍保留為備援/舊版相容。完整 OAuth 與 Render 設定請參閱 `README_V5_3.md`。
