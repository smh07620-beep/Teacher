...
class RuntimeConvergence74Tests(unittest.TestCase):
    ...
    def test_legacy_assessment_application_is_not_a_second_ui(self):
        for forbidden in (
            "legacy.classList.add('hidden')", "insertAdjacentHTML", "assessment-681-body",
            "assessment-681-tabs", "question-bank-drawer", "function examsHTML",
            "function bankHTML", "function aiHTML", "function openQuestion",
            "function saveQuestion", "function createAiDrafts",
        ):
    ...
    def test_old_question_overlay_and_admin_bundle_are_deleted(self):
        self.assertFalse((ROOT / "static" / "question-authoring-ux-71.js").exists())
        self.assertFalse((ROOT / "static" / "system-admin.js").exists())
        self.assertIn("Shared admin runtime state", self.shared)
    ...
    def test_unique_advanced_owner_does_not_reimplement_crud_or_ai(self):
        ...
        for forbidden in (
            "/api/ai-questions/generate", "/api/question-bank/drafts",
            "/api/quiz-questions/", "adminAddQuizQuestion", "adminDeleteQuizQuestion",
        ):
    ...
    def test_architecture_and_audit_freeze_single_runtime_ownership(self):
        ...
        self.assertIn("`static/system-admin.js` is deleted", self.architecture)
