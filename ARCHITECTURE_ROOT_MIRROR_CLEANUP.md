...
| Materials catalog and metadata controller | `teacher_app.materials.service`, `routes` | Existing `/api/slides*` endpoint names remain in `app.py` as thin canonical delegates; `pgy_app.py` replaces their live handlers before RBAC/wrapper registration | **Stage 5.1 converged runtime and source ownership.** Catalog projection, metadata update and delete orchestration belong to `teacher_app.materials`; provider SDK/credential/storage primitives remain legacy seams for now. |
| Courses and teaching-plan controller | `teacher_app.courses.service`, `routes` | Existing `/api/courses*` URL rules remain in `app.py` as thin canonical delegates while `pgy_app.py` installs the canonical runtime handlers | **Stage 5.1 converged runtime and source ownership.** Course CRUD, teaching-plan validation and persistence orchestration belong to `teacher_app.courses`. |
| Assessment configuration/review/publication | `teacher_app.assessments.service`, `routes` | Existing `/api/quiz-categories*` URL rules remain in `app.py` as thin canonical delegates; Question Bank/RBAC adapters may still wrap the canonical handlers | **Stage 5.1 converged runtime and source ownership.** Category CRUD, review, publication snapshot orchestration and material-link rules belong to `teacher_app.assessments`. |
