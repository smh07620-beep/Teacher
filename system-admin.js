...
async function renderMaterialJobs(force=false){
    ...
    try{
        ...
        const jobs=d.jobs||[],b=d.r2Budget||{},level=String(b.level||'green'),levelStyle=level==='emergency'?'text-rose-800 bg-rose-100 border-rose-200':level==='blocked'?'text-rose-700 bg-rose-50 border-rose-200':level==='warning'?'text-amber-800 bg-amber-100 border-amber-200':'text-emerald-800 bg-emerald-50 border-emerald-200';
        const budget=`<div class="rounded-xl border border-sky-200 bg-white p-3 text-[11px]"><div class="flex items-center justify-between gap-2 flex-wrap"><b class="text-sky-950">R2 免費額度（Teacher estimate）</b><span class="border rounded-full px-2 py-0.5 font-bold ${levelStyle}">${escapeHtml(level)}</span></div><div class="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2 text-slate-600"><span>額度 ${Number(b.freeStorageGbMonth||0).toFixed(2)} GB-month</span><span>估算 ${Number(b.estimatedGbMonth||0).toFixed(4)} GB-month</span><span>使用 ${Number(b.usagePercent||0).toFixed(1)}%</span><span>暫存 ${formatFileBytes(b.currentStagingBytes||0)}</span><span>保留 ${formatFileBytes(b.reservedBytes||0)}</span><span>進行上傳 ${Number(b.activeUploads||0)}</span><span>待清理 ${Number(b.cleanupPending||0)}</span><span>parts ${Number(b.multipartParts||0)}／ops ${Number(b.estimatedOperations||0)}</span></div><div class="mt-2 text-slate-500">佇列：pending ${Number(d.pendingJobs||0)} · processing ${Number(d.processingJobs||0)} · retry ${Number(d.retryJobs||0)} · failed ${Number(d.failedJobs||0)}</div></div>`;
        const jobMarkup=jobs.length?jobs.map(j=>{const pct=Math.max(0,Math.min(100,Number(j.progress||0))), retry=j.status==='failed'&&j.attempts>=j.maxAttempts;return `<div class="rounded-xl border bg-white p-3"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><div class="flex items-center gap-2 flex-wrap"><span class="text-[10px] border rounded-full px-2 py-0.5 font-bold ${materialJobStatusClass(j.status)}">${escapeHtml(materialJobStatusLabel(j.status))}</span><b class="text-xs text-slate-800 truncate">${escapeHtml(j.result?.title||j.title||j.result?.filename||j.originalName||j.id)}</b><span class="text-[10px] text-slate-400">${formatFileBytes(j.sourceBytes||0)}</span></div><p class="text-[11px] text-slate-600 mt-1">${escapeHtml(j.stage||'')}｜${escapeHtml(j.detail||'')}</p>${j.error?`<p class="text-[10px] text-rose-600 mt-1">${escapeHtml(j.error)}</p>`:''}</div><div class="flex gap-1 shrink-0">${retry?`<button onclick="retryMaterialJob('${escapeHtml(j.id)}')" class="text-[10px] px-2 py-1 rounded-lg bg-amber-100 hover:bg-amber-200 text-amber-800 font-bold">重新處理</button>`:''}${['queued','retry_wait'].includes(j.status)?`<button onclick="cancelMaterialJob('${escapeHtml(j.id)}')" class="text-[10px] px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600">取消</button>`:''}</div></div><div class="mt-2 h-1.5 rounded-full bg-slate-100 overflow-hidden"><div class="h-full bg-sky-500 transition-all" style="width:${j.status==='completed'?100:pct}%"></div></div><div class="mt-1 text-[10px] text-slate-400">工作 ${escapeHtml(j.id)} · 第 ${Number(j.attempts||0)}/${Number(j.maxAttempts||3)} 次${j.materialId?` · 教材 ${escapeHtml(j.materialId)}`:''}</div></div>`}).join(''):'<p class="text-xs text-slate-400">目前沒有背景教材工作。</p>';
        host.innerHTML=budget+jobMarkup;
