...
- Production entrypoint remains `pgy_app:app`.
- `pgy_app.py` is composition only: it registers compatibility adapters and canonical modules, but it must not become a second business-logic host.
- `app.py` remains the legacy Flask compatibility host while runtime ownership moves behind `teacher_app/` modules.
...
| --- | --- | --- |
| Auth and session lifecycle | `teacher_app.auth.service`, `teacher_app.auth.repository`, `teacher_app.auth.routes` | `app.py` exposes only the legacy route names and thin delegates needed for compatibility. **Frozen / converged.** |
...
| Courses and teaching plans | `teacher_app.courses.service` plus repositories | Existing URLs remain compatible while data-access migration continues where still required. |
| Course Wizard bundle/follow-up | `teacher_app.courses.bundle`, `teacher_app.courses.bundle_followup` | `course_bundle_72.py` and `course_bundle_followup_73.py` remain HTTP/handler adapters; 0072/0073 DDL is owned by `schema_migrations.py`. |
...
| Atlas | `teacher_app.atlas.service`, `teacher_app.atlas.repository` | `atlas_70.py` keeps established URLs and transport/parsing compatibility where not yet extracted. |
| External interactive media | `teacher_app.materials.external_media` | `external_media_68.py` keeps route/RBAC/response compatibility only. |
| Admin elevation | `admin_elevation_68.py` with `sensitive_elevation_69.py` | One compatibility pair; new sensitive rules extend the existing contract instead of duplicating elevation state. |
...
The current package layout establishes canonical storage deletion and worker persistence/protocol groundwork. Remaining convergence work should rewire the live upload/read and worker HTTP/executor adapters in bounded slices, preserve existing response contracts, and remove compatibility code only after all live callers have moved and release checks cover the replacement.
